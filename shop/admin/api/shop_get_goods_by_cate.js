...
function shopGetGoodsByCate() {
    ...
    this.Service = async function (version, param, callback) {
        ...
        try {
            ...
            let cate = JSON.parse(param['cate'])[1]
            let brand = JSON.parse(param['brand'])
            // 数据库需要加订单号 说明是同一个包裹
            // state -1全部 0在售 1售罄 2下架
            if (param['state'] == -1) {
                if (param['need_integral'] == 0) {
                    sql = "select count(id) from `item` where integral_price = ? and category_id_1 in (?) and brand_id in (?)";
                } else if (param['need_integral'] == 1) {
                    sql = "select count(id) from `item` where integral_price > ? and category_id_1 in (?) and brand_id in (?)";
                }

                row = await db.Query(sql, [0, cate, brand]);
                data.number = row[0]['count(id)']
                // 全部拉取
                if (param['need_integral'] == 0) {
                    sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price = ? and category_id_1 in (?)  and brand_id in (?) ORDER BY create_time desc limit ?,?";
                } else if (param['need_integral'] == 1) {
                    sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price > ? and category_id_1 in (?)  and brand_id in (?) ORDER BY create_time desc limit ?,?";
                }
                row = await db.Query(sql, [0, cate, brand, param['last_id'] * 5, 5]);
            } else if (param['state'] == 0) {
                // 出售中
                if (param['need_integral'] == 0) {
                    sql = "select count(id) from `item` where integral_price = ? and state = ? and category_id_1 in (?) and brand_id in (?)";
                } else if (param['need_integral'] == 1) {
                    sql = "select count(id) from `item` where integral_price > ? and state = ? and category_id_1 in (?) and brand_id in (?)";
                }

                row = await db.Query(sql, [0, 0, cate, brand]);
                data.number = row[0]['count(id)']

                if (param['need_integral'] == 0) {
                    sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price = ? and state = ? and category_id_1 in (?) and brand_id in (?) ORDER BY create_time desc limit ?,?";
                } else if (param['need_integral'] == 1) {
                    sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price > ? and state = ? and category_id_1 in (?) and brand_id in (?)ORDER BY create_time desc limit ?,?";
                }
                row = await db.Query(sql, [0, 0, cate, brand, param['last_id'] * 5, 5]);
                ...
                if (notStockItemList.length > 0) {
                    ...
                    if (param['need_integral'] == 0) {
                        sql = "select count(id) from `item` where integral_price = ? and id in (?) and category_id_1 in (?) and brand_id in (?)";
                    } else if (param['need_integral'] == 1) {
                        sql = "select count(id) from `item` where integral_price > ? and id in (?) and category_id_1 in (?) and brand_id in (?)";
                    }
                    ...
                    if (param['need_integral'] == 0) {
                        sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price = ? and state = ? and id in (?) and category_id_1 in (?) and brand_id in (?) ORDER BY create_time desc limit ?,?";
                    } else if (param['need_integral'] == 1) {
                        sql = "select id,`name`,image,url,qcl,price,`describe`,`type`,integral_price,sort,state,specification_id_1,specification_id_2,category_id_1,category_id_2,category_id_3,create_time,brand_id,review_id,goods_info from item where integral_price > ? and state = ? and id in (?) and category_id_1 in (?) and brand_id in (?) ORDER BY create_time desc limit ?,?";
                    }
                    row = await db.Query(sql, [0, 0, notStockItemList.map(function (fn) {
                        return fn
                    }), cate, brand, param['last_id'] * 5, 5]);
                    console.info(row)
                }
                ...
                row = await db.Query(sql, [0, 1, cate, brand]);
                data.number = row[0]['count(id)']
                ...
                row = await db.Query(sql, [0, 1, cate, brand, param['last_id'] * 5, 5]);
            }
...
module.exports = shopGetGoodsByCate;