...
function shopUpdateBrand() {
    ...
    this.Service = async function (version, param, callback) {
        ...
        try {
            ...
            brandImg = brandImg[0]
            sql = "update shop_brand set image = ?,url = ?,name = ?,price = ?,user_id = ?,create_time = CURRENT_TIMESTAMP,status = ?,sort = ?,`describe` = ? where id = ?";
            row = await db.Query(sql, [brandImg, param['url'], param['name'], param['price'], param['user_id'], param['status'], param['sort'], param['desc'], param['id']]);
            console.info(row)
