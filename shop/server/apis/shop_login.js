...
function SHOPLogin() {
    ...
    this.Run = async function (ver, param, res) {
        ...
        // }
            ...
            var row = [];
            try {
                sql = "select user_name,avatar,open_id,address_id,id from shop_user where open_id = ?";
                ...
                if (row.length == 0) {
                    ...
                    if (row[0].address_id) {
                        ...
                        sql = 'select receiver,tel,province,city,area,road,id from shop_address where id = ?'
                        row = await query(sql, row[0].address_id)
                        data.address = row
                        data.address[0].isDefault = true
                    } else {
                        sql = 'select receiver,tel,province,city,area,road,id from shop_address where user_id = ?'
                    ...
                    await query("update shop_user set last_login_time = CURRENT_TIMESTAMP where id = ? ", [user_id]);
                    // }else{
