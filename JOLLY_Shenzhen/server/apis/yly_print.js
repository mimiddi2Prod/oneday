...
async function printOrder(params) {
    return new Promise(async function (resolve, reject) {
        ...
        order.forEach(m => {
            goods.forEach(n => {
                if (m.goods_id == n.id && n.machine_tag == "后厨") {
                    order2.push(m)
                }
            })
        })
        if (order2.length) {
            ...
            content2 += repeat('.', 32);
            content2 += "<FS2><center>--线上订单--</center></FS2>";
            // content += "<FS><center>Onday 森南店</center></FS>";
            content2 += "订单时间:" + formatTime(new Date(trade.create_time)) + "\n";
            content2 += "订单编号:" + trade.trade_id + "\n";
            content2 += "支付方式:" + (trade.pay_method == 'Wxpay' ? '微信支付' : '余额支付') + "\n";
            content2 += "桌号:" + (trade.table_number || '') + "\n";
            content2 += "人数:" + (trade.dinners_number || '') + "\n";
            content2 += "用餐方式:" + (trade.take_meal_style == 0 ? '堂食' : '外带') + "\n";
            ...
            // content2 += repeat('.', 32);
            // content += "<QR>this is qrcode,you can write Officical Account url or Mini Program and so on</QR>";
            // content2 += "原价:￥" + trade.goods_total_original_price + "\n";
            // content2 += "小计:￥" + trade.goods_total_price + "\n";
            // content2 += "折扣:￥" + (trade.goods_total_price - Number(trade.actually_total_price)).toFixed(2) + " \n";
            // content += (trade.pay_method == 'Balance' ? "会员余额:" + trade.balance + " \n" : '')
            content2 += repeat('*', 32);
            // content2 += "订单实付:￥" + trade.actually_total_price + "\n";
            if (trade.remark && trade.remark.length) {
