...
async function printOrder(params) {
    return new Promise(async function (resolve, reject) {
        ...
        order.forEach(m => {
            goods.forEach(n => {
                if (m.goods_id == n.id && new RegExp("后厨").test(n.machine_tag)) {
                    order2.push(m)
                }
            })
        })
        if (order2.length) {
            let order_str_2 = "", content2
            order2.forEach(m => {
                order_str_2 += "<tr><td>" + m.name + "</td><td>" + m.price + "</td><td>x" + m.number + "</td><td>" + m.subtotal + "</td></tr>";
                if (m.param && m.param.length) {
                    m.param = JSON.parse(m.param)
                    let text = "---"
                    for (let i in m.param) {
                    ...
                    order_str_2 += "<tr><td>" + text + "</td></tr>";
                }
                if (m.remark && m.remark.length) {
                    order_str_2 += "<tr><td>---备注:" + m.remark + "</td></tr>";
                }
            })
            content2 = "<MN>2</MN>"; // 打印两联
            content2 += "<FS2><center>Oneday 森南店</center></FS2>";
            content2 += repeat('.', 32);
            content2 += "<FS2><center>--线下订单--</center></FS2>";
            content2 += "订单时间:" + formatTime(trade.create_time) + "\n";
            content2 += "订单编号:" + trade.trade_id + "\n";
            content2 += "支付方式:" + trade.pay_method + "\n";
            content2 += "桌号:" + (trade.table_number || '') + "\n";
            content2 += "人数:" + (trade.dinners_number || '') + "\n";
            content2 += repeat('*', 14) + "商品" + repeat("*", 14);
            content2 += "<table>";
            content2 += "<tr><td>商品</td><td>单价</td><td>数量</td><td>小计</td></tr>";
            content2 += order_str_2
            content2 += "</table>";
            content2 += repeat('.', 32);
            if (trade.remark && trade.remark.length) {
                content2 += "订单备注:" + trade.remark + "\n";
            }
            content2 += "<FS2><center>**#1 完**</center></FS2>";
            let machineCode2 = yly.Machine.filter(val => {
                return val.name == "后厨"
            })[0].machine_code //一台设备
            yly.Print.index(machineCode2, originId, content2).then(function (res) {
                // console.log(res);
                resolve(res);
            });
        }
...
async function printPendingOrder(params) {
    return new Promise(async function (resolve, reject) {
        ...
        if (order2.length) {
            let order_str_2 = "", content2
            order2.forEach(m => {
                order_str_2 += "<tr><td>" + m.name + "</td><td></td><td>" + m.number + "</td></tr>";
                if (m.param && m.param.length) {
                    m.param = JSON.parse(m.param)
                    let text = "---"
                    for (let i in m.param) {
...
async function printPendingOrderAppend(params) {
    return new Promise(async function (resolve, reject) {
        ...
        if (order2.length) {
            ...
            content2 += "<FS2><center>Oneday 森南店</center></FS2>";
            content2 += repeat('.', 32);
            content2 += "<FS2><center>--" + trade.title + "--</center></FS2>";
            content2 += "打单时间:" + formatTime(new Date()) + "\n";
            content2 += "订单编号:" + trade.trade_id + "\n";
            content2 += "桌号:" + (trade.table_number || '') + "\n";
            content2 += repeat('*', 14) + "商品" + repeat("*", 14);
            content2 += "<table>";
            content2 += "<tr><td>商品</td><td></td><td>数量</td></tr>";
            content2 += order_str_2
            content2 += "</table>";
            content2 += repeat('*', 32);
            if (trade.update_remark && trade.update_remark.length) {
                content2 += (trade.update_remark ? "备注:" + trade.update_remark : null) + "\n";
            }
            content2 += "<FS2><center>**#1 完**</center></FS2>";
            let machineCode2 = yly.Machine.filter(val => {
                return val.name == "后厨"
...
async function printInvalidOrder(params) {
    return new Promise(async function (resolve, reject) {
        ...
        if (order2.length) {
            ...
            content2 += repeat('*', 32);
            if (trade.invalid_remark && trade.invalid_remark.length) {
                content2 += (trade.invalid_remark ? "备注:" + trade.invalid_remark : null) + "\n";
            }
            content2 += "<FS2><center>**#1 完**</center></FS2>";
            let machineCode2 = yly.Machine.filter(val => {
                return val.name == "后厨"
...
async function printAfterSale(params) {
    return new Promise(async function (resolve, reject) {
        ...
        if (order2.length) {
            let order_str_2 = "", content2
            order2.forEach(m => {
                order_str_2 += "<tr><td>" + m.name + "</td><td></td><td>" + m.return_number + "</td></tr>";
                if (m.param && m.param.length) {
                    m.param = JSON.parse(m.param)
                    let text = "---"
                    for (let i in m.param) {
            ...
            content2 += "</table>";
            // content2 += repeat('.', 32);
            // content2 += "小计:￥" + trade.after_sale_price + "\n";
            content2 += repeat('*', 32);
            if (trade.after_sale_remark && trade.after_sale_remark.length) {
                content2 += (trade.after_sale_remark ? "备注:" + trade.after_sale_remark : null) + "\n";
            }
            content2 += "<FS2><center>**#1 完**</center></FS2>";
            let machineCode2 = yly.Machine.filter(val => {
                return val.name == "后厨"
