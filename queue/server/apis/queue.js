...
function queue() {
    ...
    this.Run = async function (ver, param, res) {
        ...
        if (param['token'].length <= 0) {
            ...
            try {
                sql = "select id from odqueue where token = ?"
                row = await query(sql, param['token'])
                if (row.length <= 0) {
                    // 没有找到就插入
                    sql = "insert into odqueue(token,create_time,update_time)values(?,current_timestamp,current_timestamp)"
                    row = await query(sql, param['token'])
                } else {
                    // 有一直在排队 更新排队时间
                ...
                sql = "select * from odqueue where update_time > ? and update_time <= current_timestamp limit 2"
